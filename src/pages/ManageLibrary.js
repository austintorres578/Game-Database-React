...
export default function ManageLibrary() {
  ...
  const [sheetView, setSheetView] = useState(null); // null | "sort" | "game-info"
  const [activeSheetGame, setActiveSheetGame] = useState(null);
  ...
  async function handleApply(overrideAction) {
    ...
    try {
      ...
      if (action === "remove") {
        ...
        await Promise.all(
          affectedGroups.map(async (g) => {
            const updatedIds = g.gameIds.filter(
  ...
  async function handleRemoveFromGroup() {
    ...
    const removedIds = new Set(selectedGames.map((g) => String(g.id)));
    const updatedIds = (group.gameIds || []).filter(
      (id) => !removedIds.has(String(id)),
    );
  ...
  async function handleAddToGroup() {
    ...
    setGroups((prev) =>
      prev.map((g) =>
        g.id === addToGroupSelected ? { ...g, gameIds: updatedIds } : g,
      ),
    );
  ...
  async function handleGroupBulkApply() {
    ...
    if (groupBulkAction === "delete") {
      ...
      setGroups((prev) => {
        const updated = prev.filter((g) => !selectedGroupRows.has(g.id));
        const newTotal = Math.max(
          1,
          Math.ceil(updated.length / GROUPS_PER_PAGE),
        );
        setGroupPage((p) => Math.min(p, newTotal));
        return updated;
      });
  ...
  async function handleCreateGroupRow() {
    ...
    try {
      const savedGroup = await createGroupInFirestore(user.uid, {
  ...
  async function handleDeleteGroupRow(groupId) {
    ...
    setGroups((prev) => {
      ...
      const newTotal = Math.max(1, Math.ceil(updated.length / GROUPS_PER_PAGE));
      setGroupPage((p) => Math.min(p, newTotal));
      return updated;
    });
  ...
  async function handleUpdateGroupRow(groupId) {
    ...
    await setDoc(
      ...
      { name: editingGroupRowName.trim() },
      { merge: true },
    );
    setGroups((prev) =>
      prev.map((g) =>
        g.id === groupId ? { ...g, name: editingGroupRowName.trim() } : g,
      ),
    );
  ...
  const allGroupedIds = new Set(
    groups.flatMap((g) => g.gameIds?.map(String) || []),
  );
  ...
  const groupFilteredGames =
    selectedGroupIds.length === 0
      ? libInfo
      : selectedGroupIds.includes("__ungrouped__") &&
          selectedGroupIds.length === 1
        ? libInfo.filter((game) => !allGroupedIds.has(String(game.id)))
        : libInfo.filter((game) => {
            return selectedGroupIds.some((id) => {
              if (id === "__ungrouped__")
                return !allGroupedIds.has(String(game.id));
  ...
  const filteredGames = groupFilteredGames
    ...
    .sort((a, b) => {
      switch (sortBy) {
        ...
        case "meta-low":
          return (a.metacritic || 0) - (b.metacritic || 0);
        case "added-new":
  ...
  const allSelected =
    ...
    filteredGames.every((g) => selectedGames.some((s) => s.id === g.id));
  ...
  const allGroupRowsSelected =
    ...
    sortedGroups.every((g) => selectedGroupRows.has(g.id));
  ...
  const groupStats = filteredBeforeStatus.reduce(
    (acc, game) => {
      acc.total += 1;
  ...
  async function loadLibrary() {
    ...
    try {
      ...
      const libraryDocs = librarySnap.docs.map((d) => ({
        id: d.id,
        ...d.data(),
      }));
      const completedDocs = completedSnap.docs.map((d) => ({
        id: d.id,
        status: "completed",
        ...d.data(),
      }));
      ...
      const backlogCount = libraryDocs.filter(
        (g) => g.status === "backlog",
      ).length;
      const playingCount = libraryDocs.filter(
        (g) => g.status === "playing",
      ).length;
    ...
    <div className="manage-library-page">
      <div className="manage-library-page-wrapper">
        ...
        <section className="management-buttons">
          <button
            className={activeManagementTab === "library" ? "active" : ""}
            onClick={() => setActiveManagementTab("library")}
          >
            Game Library
          </button>
          <button
            className={activeManagementTab === "groups" ? "active" : ""}
            onClick={() => setActiveManagementTab("groups")}
          >
            Groups
          </button>
        </section>
        <section
          className="manage-library-con"
          style={{
            display: activeManagementTab === "library" ? undefined : "none",
          }}
        >
          <div className="manage-library-top">
            <div className="quick-actions">
              ...
              >
                ...
                <option value="unmark">Mark as Backlog</option>
            ...
            <div className="library-buttons">
              ...
              >
                All<span>{groupStats.total}</span>
              </button>
              <button
                className={statusFilter === "backlog" ? "active" : ""}
                onClick={() => {
                  setStatusFilter("backlog");
                  setCurrentPage(1);
                }}
              >
                Backlog<span>{groupStats.backlog}</span>
              </button>
              <button
              ...
              >
                Complete<span>{groupStats.completed}</span>
              </button>
              <button
                className={statusFilter === "custom" ? "active" : ""}
                onClick={() => {
                  setStatusFilter("custom");
                  setCurrentPage(1);
                }}
              >
                Custom<span>{groupStats.custom}</span>
              </button>
            </div>
            <div className="library-search">
              ...
              >
                ...
                <span
                  style={{
                    display:
                      (sortBy ? 1 : 0) + selectedGroupIds.length === 0
                        ? "none"
                        : undefined,
                  }}
                >
                  {(sortBy ? 1 : 0) + selectedGroupIds.length}
                </span>
              </button>
              {/* <select
                                multiple
              ...
              <div
                className={`group-trigger-con${desktopGroupTriggerOpen ? " active" : ""}`}
              >
                <div
                  className="group-trigger"
                  onClick={() => setDesktopGroupTriggerOpen((v) => !v)}
                >
                  <p>
                    {selectedGroupIds.length > 0 ? (
                      <>
                        Selected Groups <span>{selectedGroupIds.length}</span>
                      </>
                    ) : (
                      "All Groups"
                    )}
                ...
                <div className="group-options">
                  ...
                  <p
                    className={
                      selectedGroupIds.includes("__ungrouped__") ? "active" : ""
                    }
                  ...
                  {sortedGroups.map((g) => (
                    <p
                      key={g.id}
                      className={
                        selectedGroupIds.includes(g.id) ? "active" : ""
                      }
            ...
            <div
              className="mobile-sort-buttons-con"
              style={{
                display:
                  isMobile && selectedGames.length > 0 ? "block" : "none",
              }}
            >
              ...
              <div
                ...
                onClick={() => {
                  if (allSelected) {
                    ...
                    setSelectedGames((prev) =>
                      prev.filter((g) => !filteredIds.has(g.id)),
                    );
                  } else {
                    setSelectedGames((prev) => {
                      const next = [...prev];
                ...
                <div
                  ...
                  onClick={() => {
                    if (window.innerWidth <= 765) {
                      setActiveSheetGame(game);
                      setSheetView("game-info");
                    }
                  }}
                >
                  <div
                  ...
                  <div className="game-details-con">
                    ...
                    >
                      ...
                      <div>
                        <span
                          className={`mobile-status${game.status === "completed" ? " completed" : ""}`}
                        >
                          {game.status}
                        </span>
                  ...
                  <div className="groups-con">
                    <select>
                      {groups.filter((g) =>
                        g.gameIds?.includes(String(game.id)),
                      ).length === 0 ? (
        ...
        <section
          className="group-management-con"
          style={{
            display: activeManagementTab === "groups" ? "flex" : "none",
          }}
        >
          <div className="group-creation-con">
            ...
            <div>
              ...
              <input
                ...
                onKeyDown={(e) => {
                  if (e.key === "Enter") handleCreateGroupRow();
                }}
          ...
          <div className="right-col">
            <div className="group-actions-con">
              <div>
                <select
                  value={groupBulkAction}
                  onChange={(e) => setGroupBulkAction(e.target.value)}
                >
                  <option value="">Bulk Actions</option>
                  <option value="delete">Delete</option>
                </select>
                <button
                  onClick={handleGroupBulkApply}
                  disabled={!groupBulkAction || selectedGroupRows.size === 0}
                >
                  Apply
                </button>
              </div>
              <p
                style={{
                  color: allGroupRowsSelected ? "rgb(34 197 94)" : undefined,
                  fontWeight: allGroupRowsSelected ? "bold" : undefined,
                }}
              >
                {allGroupRowsSelected ? (
                  ...
                  <>
                    <span>{selectedGroupRows.size}</span> items
                  </>
                )}
              </p>
            </div>
                  ...
                  <div key={group.id} className="group-row">
                    ...
                    <div className="title-con">
                      <div
                        ...
                        style={{
                          display:
                            editingGroupRowId === group.id ? "none" : "flex",
                        }}
                      >
                        <p>{group.name}</p>
                        <div className="sub-actions">
                          <button
                            onClick={() => {
                              setEditingGroupRowId(group.id);
                              setEditingGroupRowName(group.name);
                            }}
                          >
                            Edit
                          </button>
                      ...
                      <div
                        ...
                        style={{
                          display:
                            editingGroupRowId === group.id ? "flex" : "none",
                        }}
                      >
                        <input
                          type="text"
                          ...
                          onChange={(e) =>
                            setEditingGroupRowName(e.target.value)
                          }
                          onKeyDown={(e) => {
                            if (e.key === "Enter")
                              handleUpdateGroupRow(group.id);
                          }}
                        />
                        <div>
                          <button
                            onClick={() => handleUpdateGroupRow(group.id)}
                          >
                            Update Group
                          </button>
                          <button onClick={() => setEditingGroupRowId(null)}>
                            Cancel
                          </button>
                        </div>
                      </div>
                ...
                <p>
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      if (!isNaN(v)) setGroupPage(v);
                    }}
                  />{" "}
                  of <span>{totalGroupPages}</span>
                </p>
              </div>
              <div className="group-pagination">
                <button
                  className={groupPage === 1 ? "disabled" : ""}
                  onClick={() => setGroupPage((p) => Math.max(1, p - 1))}
                >
                  Prev
                </button>
                <div
                  className={`page-trigger${groupPageTriggerOpen ? " active" : ""}`}
                  onClick={() => setGroupPageTriggerOpen((v) => !v)}
                >
                  <p>
                    Page <span>{groupPage}</span> of{" "}
                    <span>{totalGroupPages}</span>
                  </p>
                ...
                <button
                  className={groupPage === totalGroupPages ? "disabled" : ""}
                  onClick={() =>
        ...
        <section
          className="library-table-bottom library-management-pagnation"
          style={{
            display: activeManagementTab === "library" ? undefined : "none",
          }}
        >
          ...
          <div className="library-table-pagination">
            ...
            <div className="pagn-con">
              ...
              <div
                className="pagn-options"
                style={{ display: pagnOpen ? undefined : "none" }}
              >
                {(() => {
                  const half = 3;
      ...
      <div
        className="add-to-group-modal-con"
        style={{
          opacity: addToGroupModalOpen ? 1 : 0,
          pointerEvents: addToGroupModalOpen ? "all" : "none",
        }}
      >
      ...
      <div
        ...
        style={{
          ...
          pointerEvents: isApplying ? "all" : "none",
        }}
      >
      ...
      <div
        className="group-removal-modal-con"
        style={{
          opacity: groupRemovalModalOpen ? 1 : 0,
          pointerEvents: groupRemovalModalOpen ? "all" : "none",
        }}
      >
      ...
      <div
        ...
        style={{
          opacity: sheetView !== null ? 1 : 0,
          pointerEvents: sheetView !== null ? "all" : "none",
        }}
        onClick={() => closeSheet()}
      >
        <div
          className="mobile-hidden-menu-wrapper"
          style={{
            transform:
              sheetView !== null
                ? "translate(-50%, -35%)"
        ...
        >
          ...
          {sheetView === "sort" && (
            <div className="sort-options">
                ...
                <div className="options-wrapper">
                  ...
                  >
                    ...
                    <div
                      ...
                      style={{
                        visibility: sortBy === "name-az" ? "visible" : "hidden",
                      }}
                    >
                      <span>✓</span>
                    </div>
                  ...
                  <button
                    className={sortBy === "name-za" ? "active" : ""}
                    onClick={() => {
                      setSortBy("name-za");
                      setCurrentPage(1);
                    }}
                  >
                    ...
                    <div
                      className="enabled-sym"
                      style={{
                        visibility: sortBy === "name-za" ? "visible" : "hidden",
                      }}
                    >
                      <span>✓</span>
                    </div>
                  ...
                  <button
                    className={sortBy === "meta-high" ? "active" : ""}
                    onClick={() => {
                      setSortBy("meta-high");
                      setCurrentPage(1);
                    }}
                  >
                    ...
                    <div
                      ...
                      style={{
                        visibility:
                          sortBy === "meta-high" ? "visible" : "hidden",
                      }}
                    >
                      <span>✓</span>
                    </div>
                  ...
                  <button
                    className={sortBy === "meta-low" ? "active" : ""}
                    onClick={() => {
                      setSortBy("meta-low");
                      setCurrentPage(1);
                    }}
                  >
                    ...
                    <div
                      ...
                      style={{
                        visibility:
                          sortBy === "meta-low" ? "visible" : "hidden",
                      }}
                    >
                      <span>✓</span>
                    </div>
                  ...
                  <button
                    className={sortBy === "added-new" ? "active" : ""}
                    onClick={() => {
                      setSortBy("added-new");
                      setCurrentPage(1);
                    }}
                  >
                    ...
                    <div
                      ...
                      style={{
                        visibility:
                          sortBy === "added-new" ? "visible" : "hidden",
                      }}
                    >
                      <span>✓</span>
                    </div>
                  ...
                  <button
                    className={sortBy === "added-old" ? "active" : ""}
                    onClick={() => {
                      setSortBy("added-old");
                      setCurrentPage(1);
                    }}
                  >
                    ...
                    <div
                      ...
                      style={{
                        visibility:
                          sortBy === "added-old" ? "visible" : "hidden",
                      }}
                    >
                      <span>✓</span>
                    </div>
              ...
              <div className="option-group">
                <span>Groups</span>
                <div
                  className={`group-trigger-con${groupTriggerOpen ? " active" : ""}`}
                >
                  <div
                    className="group-trigger"
                    onClick={() => setGroupTriggerOpen((v) => !v)}
                  >
                    <p>
                      {selectedGroupIds.length > 0 ? (
                        <>
          ...
          {sheetView === "game-info" && activeSheetGame && (
            <div className="mobile-game-info">
              <div className="title-con">
                <div className="title">
                  <h3>{activeSheetGame.title}</h3>
                  <span>{activeSheetGame.metacritic ?? "—"}</span>
                </div>
                <div className="group-row">
                  <span>Groups</span>
                  <div>
                    {groups.filter((g) =>
                      g.gameIds?.includes(String(activeSheetGame.id)),
                    ).length === 0 ? (
                      <p>Ungrouped</p>
                    ) : (
                      groups
                ...
                <div className="platform-row">
                  <span>Platforms</span>
                  <div>
                    {(Array.isArray(activeSheetGame.platforms)
                      ? activeSheetGame.platforms
                      : [activeSheetGame.platforms]
                    )
                      .filter(Boolean)
                      .map((p, i) => (
                ...
                <div className="change-status-con">
                  ...
                  >
                    ...
                    {activeSheetGame.status === "completed" && <p>✓</p>}
                  </div>
                  <div
                    className="change-status-action"
                    onClick={async () => {
                  ...
                  >
                    ...
                    {activeSheetGame.status === "backlog" && <p>✓</p>}
                  </div>
                </div>
